...
#### 3.3.2 开发“公共API接口”（参考xxl-rpc-example-api）
- 1、开发一个API接口
...
#### 3.3.3 开发“服务提供方”（参考xxl-rpc-example-server）
...
#### 3.3.4 开发“服务消费方”（参考xxl-rpc-example-client）
- 1、配置maven依赖：XXL-RPC核心依赖 + 公共API接口依赖
...
- 1、包名重构；
- 2、模块化划分；
- 3、ZK注册中心重构，不依赖配置文件，通过代码初始化；
- 4、各模块提供默认选型，备选项需要单独依赖进入；最小精简选型组合 "jetty + hessian"
- 5、服务AccessToken鉴权；
- 6、【ING】支持多种请求方式，如：SYNC、ONEWAY、FUTURE、CALLBACK 等；
...
- 服务注册中心, 节点支持单个移除
- 通讯组件处Netty + Hessian之外，其他置为非强制依赖；
- 推送core到maven中央仓库；
