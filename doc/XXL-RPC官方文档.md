...
- 2、XXL-RPC注册中心：底层抽象注册中心模块，并原生提供自研基于DB的注册中心，真正实现开箱即用，更轻量级、降低第三方依赖；至今XXL-RPC以提供三种注册中心具体实现：Local方案、ZK方案、XXL-RPC原生注册中心方案；
    - 轻量级：基于DB与磁盘文件，只需要提供一个DB实例即可，无第三方依赖；
    - 实时性：新服务上线、下线，可以在1s内推送给客户端；
    - 数据同步：注册中心内部10s会全量同步一次磁盘数据，清理无效服务，确保服务数据实时可用；
    - 性能：服务发现时仅读磁盘文件，性能非常高；
    - 扩展性：可方便、快速的横向扩展，只需保证 "xxl-rpc-admin" 配置一致即可，可截止负载均衡组件如Nginx轻易扩展；
    - 多状态：正常=支持动态注册、发现；锁定=人工维护注册信息，固定不变；禁用=禁止实用，获取不到注册信息；
- 3、[迭代中]注册中心参数调整，IP、PORT取消默认值，原生动态获取；注册中心启动参数位置调整，与注册中心实现关联；
- 4、[迭代中]XXL-RPC客户端可快速接入，只需要切换注册中心实现为 "***" 即可；
    - 注册/摘除api：内置线程10s一次
